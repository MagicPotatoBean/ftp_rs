    ...
    if let Some(name) = packet.path() {
        ...
        if let Ok(now) = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH) {
            ...
            if file_location
                ...
                || name.contains("*")
            {
                http_log!("Request rejected: \"{path}/{name}\"");
                ...
                if std::fs::create_dir(&dir_location).is_ok() {
                    ...
                    http_log!("Failed to create folder \"{}\"", dir_location.display());
                }
            }
        } else {
            http_log!("Failed to get system time");
        }
    }
    packet.read_all();
    http_log!("{packet}\n");
...
pub fn get(mut packet: HttpRequest, address: SocketAddr, path: &'static str) {
    if let Some(name) = packet.path() {
        ...
        if name == "" || name == "styles.css" || name == "script.js" || name == "favicon.ico" {
            ...
            if let Some((username, path)) = name.split_once("") {
                if let Ok(file_location) = PathBuf::from("./static/users")
                    .join(username)
                    .join("files")
                    .join(path)
                    .canonicalize()
                {
                    if file_location
                        .starts_with(PathBuf::from("./static/users").join(username).join("files"))
                        && username.chars().all(|chr| chr.is_ascii_alphanumeric())
                    {
                            ...
                            || name.contains("*")
                        {
                                ...
                                std::fs::OpenOptions::new().read(true).open(file_location)
                            {
                                ...
                                http_log!("Client requested non-existent file \"{name}\"");
                            }
                        }
                    } else {
                        packet.respond_string( &format!("HTTP/1.1 410 Gone\r\n\r\nFailed to fetch \"{name}\", this is likely because it doesn't exist.\r\n")).unwrap();
    ...
    if let Some(name) = packet.path() {
            ...
            || name.contains("*")
        {
            ...
            if std::fs::remove_dir_all(&dir_location).is_ok_and(|_| {
                http_log!("File {} deleted", dir_location.display());
                true
            }) {
