...
fn main() {
    ...
    std::thread::Builder::new().name("FTP Server".to_owned()).spawn(move || ftp::host_server(ftp_addr, 128, PASSWORD_SALT, PROTECTED_NAMES).unwrap()).unwrap();
    std::thread::Builder::new().name("HTTP Server".to_owned()).spawn(move || http::host_server(http_addr, 128).unwrap()).unwrap();
